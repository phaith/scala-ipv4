...
class IpNetworkExample extends WordSpec with ShouldMatchers {
  "An ip network" should {
    ...
    "align with the longest matching prefix" in {
      val newNetwork = new IpNetwork(new IpAddress("192.168.0.3"), new IpAddress("192.168.0.250"))
      newNetwork should be(network)
    }
